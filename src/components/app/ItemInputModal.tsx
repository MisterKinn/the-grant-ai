...
export function ItemInputModal({
    ...
    const [selectedGrantType, setSelectedGrantType] =
        useState<GrantType>("PRE_STARTUP");
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            try {
                ...
                // Extract text from PDF (parse-pdf defaults to project_files bucket)
    ...
    const handleHwpxFileSelect = async (
        ...
        try {
            ...
            if (!parseResult.success) {
                toast({
                    ...
                    title: "HWPX 파싱 실패",
                    description:
                        parseResult.error || "파일 형식을 확인해주세요.",
                });
                setModalStep("select");
                return;
            }
            ...
            const items: ChecklistItem[] = parseResult.placeholders
                .slice(0, 10)
                .map((p) => ({
                    id: p.id,
                    label: p.label,
            ...
            const parsedContent =
                ...
                parseResult.contentBlocks.length > 0
            ...
            if (doc) {
                ...
                if (parsedData?.hwpxTemplatePath) {
                    await supabase
                        ...
                        .update({
                            plain_text:
                                parsedData.parsedContentWithImages ||
                                parsedData.parsedPlainText,
                            hwpx_template_path:
                                parsedData.modifiedTemplatePath ||
                                parsedData.hwpxTemplatePath,
                        })
                        .eq("id", doc.id);
                }
                ...
                const checklistContext = checkedItems
                    .map((item) => `- ${item.label}`)
                    .join("\n");
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const supportType =
                selectedGrantType === "EARLY_STARTUP"
                    ? "early_startup"
                    : "preliminary";
            ...
            if (doc) {
                ...
                if (extractedPdfText) {
                    const truncatedPdfText =
                        extractedPdfText.length > MAX_PDF_TEXT_LENGTH
                            ? extractedPdfText.substring(
                                  0,
                                  MAX_PDF_TEXT_LENGTH,
                              ) + "\n\n... (이하 생략)"
                            : extractedPdfText;
        ...
        <div className="py-4 space-y-5">
                    ...
                    {GRANT_TYPE_OPTIONS.map((option) => (
                        <button
                            ...
                            className={cn(
                                ...
                                selectedGrantType === option.value &&
                                    !option.disabled
            ...
            {selectedGrantType === "CUSTOM" && (
                <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground mb-2">
                        HWPX 파일을 업로드하면 AI가 분석하여 작성해야 할 항목을
                        안내합니다.
                    </p>
            ...
            {selectedGrantType !== "CUSTOM" && (
                <Button
                    ...
                    disabled={
                        !itemDescription.trim() || isCreating || isExtractingPdf
                    }
                    className="w-full"
                >
                    {isCreating ? (
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-lg">
                ...
                {(modalStep === "select" || modalStep === "creating") &&
                    renderSelectStep()}
            </DialogContent>
        </Dialog>
    );
