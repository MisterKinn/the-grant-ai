...
function buildChangesMap(
  ...
  console.log(`[export] Original text nodes: ${originalTexts.length}, Edited cells: ${editedCells.length}`);
  ...
  for (let i = 0; i < Math.min(originalTexts.length, editedCells.length); i++) {
...
async function applyMinimalChanges(
  ...
  for (const sectionFile of sectionFiles) {
    ...
    if (changesMap.size === 0) {
      console.log(`[export] No changes detected for ${sectionFile}, keeping original`);
      continue;
    }
    ...
    for (const [originalText, newText] of changesMap) {
      ...
      newXmlContent = newXmlContent.replace(pattern, `$1${escapeXml(newText)}$2`);
    }
    
    zip.file(sectionFile, newXmlContent);
    console.log(`[export] Applied ${changesMap.size} changes to ${sectionFile}`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    // Get request data
    const { templatePath, editedContent, title, passthrough } = await req.json();
    ...
    console.log(`[export-custom-hwpx] Exporting: ${templatePath}, passthrough: ${passthrough}`);
    ...
    if (downloadError || !fileData) {
      console.error(`[export-custom-hwpx] Download error:`, downloadError);
      return new Response(
    ...
    console.log(`[export-custom-hwpx] Original file size: ${originalArrayBuffer.byteLength} bytes`);

    // PASSTHROUGH MODE: Just return the original file as-is (for testing)
    if (passthrough) {
      console.log(`[export-custom-hwpx] Passthrough mode - returning original file`);
    ...
    console.log(`[export-custom-hwpx] ZIP loaded, files:`, Object.keys(zip.files).length);
    ...
    if (editedContent) {
      for (const sectionFile of sectionFiles) {
        ...
        zip.file(sectionFile, newXmlContent, { compression: "DEFLATE" });
        console.log(`[export] Applied ${changesMap.size} changes to ${sectionFile}`);
      }
    }
    ...
    console.log(`[export-custom-hwpx] Generated HWPX size: ${modifiedHwpx.byteLength} bytes`);
